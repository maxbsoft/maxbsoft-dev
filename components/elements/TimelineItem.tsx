import { ResumeItemModel } from '@/models';
import { Locale } from '@/types';
import { getStringFromLocalizedText } from '@/utils';

export interface TimelineItemProps {
  timeline: ResumeItemModel;
  locale: Locale;
}
const TimelineItem = ({
  ...
  return (
    <div className="timeline relative block items-start rounded-full border-white border-opacity-20 first:mt-0 md:flex">
      <span className="timeline-year absolute top-0 left-12 mt-7 mr-12 min-w-[115px] rounded-full bg-primary bg-opacity-10 p-1 text-center text-sm leading-none md:relative md:left-0">
        {getStringFromLocalizedText(year, locale)}
      </span>
      <div className="timeline-right relative border-l-4 border-white border-opacity-20 pt-16 pl-12 md:pt-6">
        ...
        <h5 className="mb-0 text-primary">{getStringFromLocalizedText(title, locale)}</h5>
        <p className="mb-2 text-heading">{getStringFromLocalizedText(meta, locale)}</p>
        {Array.isArray(text) && (
          <ul>
            {text.map((item, index) => (
              <li key={`id${index}`}>{getStringFromLocalizedText(item, locale)}</li>
            ))}
          </ul>
        )}

        {!!text && !Array.isArray(text) && <p>{text}</p>}
      </div>
    </div>
  );
};
